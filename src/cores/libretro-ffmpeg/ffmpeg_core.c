...
//#define FFMPEG3 ((LIBAVUTIL_VERSION_INT < (56, 6, 100)) || (LIBAVCODEC_VERSION_INT < AV_VERSION_INT(58, 10, 100)))
#define FFMPEG3 0
...
#if ENABLE_HW_ACCEL
      { "ffmpeg_hw_decoder", "Use Hardware decoder (restart); auto|off|dxva2|"
         "cuda|d3d11va|drm|mediacodec|opencl|qsv|vaapi|vdpau|videotoolbox" },
#endif
...
static void decode_thread(void *data)
{
   ...
   for (i = 0; (int)i < (audio_streams_num > 8 ? 8 : audio_streams_num); i++)
   {
      swr[i] = swr_alloc();
