   ...
   materialui_handle_t* mui, enum materialui_color_theme color_theme)
{
   ...
   mui->colors.header_text_active = (current_theme->nav_bar_icon_passive << 8) | 0xFF;
   ...
   memcpy(msg_queue_task_progress_2, mui->colors.list_highlighted_background,
      sizeof(msg_queue_task_progress_2));
}
   ...
   bool is_idle)
{
   ...
   /*gfx_animation_t* p_anim = anim_get_ptr();
   if ((is_idle || !GFX_DISPLAY_GET_UPDATE_PENDING(p_anim, p_disp))) {
      if (!was_on && string_is_empty(global_msg_new)) {
         return;
      }
   }*/
   ...
   math_matrix_4x4* mymat)
{
   ...
   if (menu_core_enable)
   {
      char core_title[1024];
      char core_title_buf[1024];

      core_title[0] = '\0';
      core_title_buf[0] = '\0';
      if (global_msg_new && !string_is_empty(global_msg_new))
      {
         strlcpy(core_title, global_msg_new, sizeof(core_title));
         was_on = true;
      }
      else {
   ...
   if (string_is_empty(global_msg_new) && was_on)
   {
