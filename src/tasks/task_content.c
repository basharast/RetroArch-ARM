...
#else
                     basedir_attribs = INVALID_FILE_ATTRIBUTES;
                     WIN32_FILE_ATTRIBUTE_DATA lpFileInfo;
                     BOOL state = GetFileAttributesExFromAppW(utf8_to_utf16_string_alloc(new_basedir), GetFileExInfoStandard, &lpFileInfo);
                     if (state) {
                        basedir_attribs = lpFileInfo.dwFileAttributes;
                     }
#endif
...
#else
                        if (!CreateDirectoryFromAppW(utf8_to_utf16_string_alloc(new_basedir), NULL)) {
#endif
                           if (!CreateDirectoryUWP(new_basedir, false)) {
                              strlcpy(new_basedir, uwp_dir_data, sizeof(new_basedir));
                           }
                        }
                     }
...
#if defined(_M_ARM)
                  if (!CopyFile(utf16_to_utf8_string_alloc(wcontent_path), utf16_to_utf8_string_alloc(wnew_path), false))
#else
                  if (!CopyFileFromAppW(wcontent_path, wnew_path, false))
#endif
                  {
                     if (!CopyUWP2(wcontent_path, wnew_path)) {
