...
static bool gl3_init_hw_render(gl3_t *gl, unsigned width, unsigned height)
{
   ...
   if (hwr->depth)
   {
      RARCH_WARN("[GLCore]: Framebuffer depth is active.\n");
      glGenRenderbuffers(1, &gl->hw_render_rb_ds);
      glBindRenderbuffer(GL_RENDERBUFFER, gl->hw_render_rb_ds);
      glRenderbufferStorage(GL_RENDERBUFFER, hwr->stencil ? GL_DEPTH24_STENCIL8 : GL_DEPTH_COMPONENT16, width, height);
      glBindRenderbuffer(GL_RENDERBUFFER, 0);

      if (hwr->stencil) {
         RARCH_WARN("[GLCore]: Framebuffer stencil is active.\n");
