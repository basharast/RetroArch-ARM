...
void set_primitive_topology(d3d11_video_t* d3d11, D3D11_PRIMITIVE_TOPOLOGY topology)
{
   if (current_topology != topology)
   {
      d3d11->context->lpVtbl->IASetPrimitiveTopology(d3d11->context, topology);
...
#if IS_LEVEL_93
static void d3d11_overlay_vertex_geom(void* data, unsigned index, float x, float y, float w, float h)
{
   ...
   HRESULT hr = d3d11->context->lpVtbl->Map(
      d3d11->context, (D3D11Resource)d3d11->overlays.vbo, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mapped_vbo);

   if (FAILED(hr))
   {
      // Handle mapping failure
      RARCH_ERR("Failed to map vertex buffer: %d\n", hr);
      return;
   }

   d3d11_sprite_t* sprite = (d3d11_sprite_t*)mapped_vbo.pData + index * 6;

   // Set up two triangles for the quad
   sprite[0].x = x;          sprite[0].y = y;          sprite[0].w = w; sprite[0].h = h; sprite[0].u = 0.0f; sprite[0].v = 0.0f;
   sprite[1].x = x + w;      sprite[1].y = y;          sprite[1].w = w; sprite[1].h = h; sprite[1].u = 1.0f; sprite[1].v = 0.0f;
   sprite[2].x = x;          sprite[2].y = y + h;      sprite[2].w = w; sprite[2].h = h; sprite[2].u = 0.0f; sprite[2].v = 1.0f;
   sprite[3].x = x;          sprite[3].y = y + h;      sprite[3].w = w; sprite[3].h = h; sprite[3].u = 0.0f; sprite[3].v = 1.0f;
   sprite[4].x = x + w;      sprite[4].y = y;          sprite[4].w = w; sprite[4].h = h; sprite[4].u = 1.0f; sprite[4].v = 0.0f;
   sprite[5].x = x + w;      sprite[5].y = y + h;      sprite[5].w = w; sprite[5].h = h; sprite[5].u = 1.0f; sprite[5].v = 1.0f;

   for (int i = 0; i < 6; ++i)
   {
      sprite[i].scaling = 1.0f;
      sprite[i].rotation = 0.0f;
   }

   d3d11->context->lpVtbl->Unmap(d3d11->context, (D3D11Resource)d3d11->overlays.vbo, 0);

   // RARCH_LOG("Vertex %d: x=%f, y=%f, w=%f, h=%f\n", index, x, y, w, h);
}
   ...
   float x, float y, float w, float h)
{
      ...
      d3d11->context, (D3D11Resource)d3d11->overlays.vbo, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mapped_vbo);
   {
      ...
      sprites[index].params.scaling = 1.0f;
      sprites[index].params.rotation = 0.0f;
   }
   d3d11->context->lpVtbl->Unmap(d3d11->context, (D3D11Resource)d3d11->overlays.vbo, 0);
}
#endif
...
#if IS_LEVEL_93
// This function is not in use in general in RetroArch
static void d3d11_overlay_tex_geom(void* data, unsigned index, float u, float v, float s, float t)
{
   D3D11_MAPPED_SUBRESOURCE mapped_vbo;
   d3d11_video_t* d3d11 = (d3d11_video_t*)data;
   ...
   // Set up texture coordinates for the quad
   sprite[0].u = u;          sprite[0].v = v;
   sprite[1].u = u + s;      sprite[1].v = v;
   sprite[2].u = u;          sprite[2].v = v + t;
   sprite[3].u = u;          sprite[3].v = v + t;
   sprite[4].u = u + s;      sprite[4].v = v;
   sprite[5].u = u + s;      sprite[5].v = v + t;
...
static void d3d11_overlay_set_alpha(void* data, unsigned index, float mod)
{
   ...
   for (int i = 0; i < 6; ++i)
   {
      for (int j = 0; j < 4; ++j)
      {
         sprite[i].colors[j][0] = 1.0f; // Red
         sprite[i].colors[j][1] = 1.0f; // Green
         sprite[i].colors[j][2] = 1.0f; // Blue
         sprite[i].colors[j][3] = mod;  // Alpha, mod applied here
      }
   }
...
static bool d3d11_overlay_load(void* data, const void* image_data, unsigned num_images)
{
   ...
   // Ensure the buffer size is correct: 6 vertices per sprite
   desc.ByteWidth = sizeof(d3d11_sprite_t) * num_images * 6;
   ...
   HRESULT hr = d3d11->device->lpVtbl->CreateBuffer(d3d11->device, &desc, NULL, &d3d11->overlays.vbo);
   if (FAILED(hr)) {
   ...
   hr = d3d11->context->lpVtbl->Map(d3d11->context, (D3D11Resource)d3d11->overlays.vbo, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_vbo);
   if (FAILED(hr)) {
      Release(d3d11->overlays.vbo);
   ...
   for (i = 0; i < num_images; i++)
   {
      ...
      if (d3d11->overlays.textures[i].staging)
         d3d11_update_texture(d3d11->context, images[i].width, images[i].height, 0, DXGI_FORMAT_B8G8R8A8_UNORM, images[i].pixels, &d3d11->overlays.textures[i]);

      // Populate vertices for the current sprite
      for (int j = 0; j < 6; ++j) {
         sprites[i * 6 + j] = (d3d11_sprite_t){
             .x = (j == 1 || j == 4 || j == 5) ? 1.0f : 0.0f,
             .y = (j >= 2) ? 1.0f : 0.0f,
             .w = 1.0f,
             .h = 1.0f,
             .u = (j == 1 || j == 4 || j == 5) ? 1.0f : 0.0f,
             .v = (j >= 2) ? 1.0f : 0.0f,
             .s = 1.0f,
             .t = 1.0f,
             .scaling = 1.0f,
         ...
         for (int k = 0; k < 4; ++k)
         {
            sprites[i * 6 + j].colors[k][0] = 1.0f;
            sprites[i * 6 + j].colors[k][1] = 1.0f;
            sprites[i * 6 + j].colors[k][2] = 1.0f;
            sprites[i * 6 + j].colors[k][3] = 1.0f;
         }
      }
   }
...
static bool d3d11_overlay_load(void* data, const void* image_data, unsigned num_images)
{
   if (!data || num_images == 0) // Check if num_images is 0 or data is NULL
      return false;
   ...
   if (!d3d11 || !images) // Additional check for images being NULL
      return false;
   ...
   HRESULT hr = d3d11->device->lpVtbl->CreateBuffer(d3d11->device, &desc, NULL, &d3d11->overlays.vbo);
   if (FAILED(hr)) {
      free(d3d11->overlays.textures); // Clean up on failure
      d3d11->overlays.textures = NULL;
      return false;
   }
   ...
   hr = d3d11->context->lpVtbl->Map(d3d11->context, (D3D11Resource)d3d11->overlays.vbo, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_vbo);
   if (FAILED(hr)) {
      Release(d3d11->overlays.vbo); // Clean up on failure
      d3d11->overlays.vbo = NULL;
      free(d3d11->overlays.textures);
      d3d11->overlays.textures = NULL;
      return false;
...
#if IS_LEVEL_93
void print_vertex(int i, const d3d11_sprite_t* vertex)
{
   RARCH_LOG("Vertex %d: x=%f, y=%f, w=%f, h=%f, u=%f, v=%f, s=%f, t=%f, scaling=%f, rotation=%f\n", i,
      vertex->x, vertex->y, vertex->w, vertex->h,
      vertex->u, vertex->v, vertex->s, vertex->t,
      vertex->scaling, vertex->rotation);
   RARCH_LOG("Colors: \n");
   for (int i = 0; i < 4; ++i)
   {
      RARCH_LOG("\t[%d]: r=%f, g=%f, b=%f, a=%f\n",
         i, vertex->colors[i][0], vertex->colors[i][1], vertex->colors[i][2], vertex->colors[i][3]);
   }
}
void debug_sprites(d3d11_video_t* d3d11) {
   // Read back data for verification
   int num_images = d3d11->overlays.count;
   D3D11_BUFFER_DESC staging_desc;
   staging_desc.ByteWidth = sizeof(d3d11_sprite_t) * num_images * 6;
   ...
   HRESULT hr = d3d11->device->lpVtbl->CreateBuffer(d3d11->device, &staging_desc, NULL, &staging_buffer);
   if (FAILED(hr)) {
      RARCH_LOG("Failed to create staging buffer. HRESULT: 0x%08X\n", hr);
      return false;
   }

   // Copy the data from the dynamic buffer to the staging buffer
   d3d11->context->lpVtbl->CopyResource(d3d11->context, (D3D11Resource)staging_buffer, (D3D11Resource)d3d11->overlays.vbo);

   // Map the staging buffer for reading
   D3D11_MAPPED_SUBRESOURCE mapped_staging;
   hr = d3d11->context->lpVtbl->Map(d3d11->context, (D3D11Resource)staging_buffer, 0, D3D11_MAP_READ, 0, &mapped_staging);
   if (FAILED(hr)) {
      RARCH_LOG("Failed to map staging buffer for reading. HRESULT: 0x%08X\n", hr);
      staging_buffer->lpVtbl->Release(staging_buffer);
      return false;
   }

   d3d11_sprite_t* sprite_check = (d3d11_sprite_t*)mapped_staging.pData;
   for (int i = 0; i < num_images * 6; ++i)
   {
      print_vertex(i, &sprite_check[i]);
   }

   d3d11->context->lpVtbl->Unmap(d3d11->context, (D3D11Resource)staging_buffer, 0);
   staging_buffer->lpVtbl->Release(staging_buffer);
}
static void d3d11_render_overlay(d3d11_video_t* d3d11)
{
   ...
   for (i = 0; i < d3d11->overlays.count; i++)
   {
      ...
      d3d11->context->lpVtbl->Draw(d3d11->context, 6, i * 6);
   }
}
...
static bool d3d11_gfx_set_shader(void* data, enum rarch_shader_type type, const char* path)
{
   ...
   case D3D_FEATURE_LEVEL_9_3:
      shader_model = 40; // Shader Model 4.0 is supported by Feature Level 9.3
      feat_level_hint = D3D11_FEATURE_LEVEL_HINT_9_3;
      break;
   case D3D_FEATURE_LEVEL_9_1:
      shader_model = 40; // Shader Model 4.0 is supported by Feature Level 9.1
      feat_level_hint = D3D11_FEATURE_LEVEL_HINT_9_1;
      break;
   default:
      feat_level_hint = D3D11_FEATURE_LEVEL_HINT_9_3;
      break;
...
#include "d3d_shaders/opaque_sm4.hlsl.h"
         ;

#if IS_LEVEL_93
      if (!d3d11_init_shader(
         d3d11->device, shader2, sizeof(shader2), NULL, "VSMain", "PSMain", NULL, desc,
...
#else
      if (!d3d11_init_shader(
         d3d11->device, shader1, sizeof(shader1), NULL, "VSMain", "PSMain", NULL, desc,
         countof(desc), &d3d11->shaders[VIDEO_SHADER_STOCK_BLEND],
         D3D11_FEATURE_LEVEL_HINT_DONTCARE
      ))
         goto error;
...
#if IS_LEVEL_93
      D3D11_INPUT_ELEMENT_DESC desc[] = {
    { "POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(d3d11_sprite_t, x), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "TEXCOORD", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(d3d11_sprite_t, u), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(d3d11_sprite_t, colors[0]), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "COLOR", 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(d3d11_sprite_t, colors[1]), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "COLOR", 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(d3d11_sprite_t, colors[2]), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "COLOR", 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(d3d11_sprite_t, colors[3]), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "SCALING", 0, DXGI_FORMAT_R32_FLOAT, 0, offsetof(d3d11_sprite_t, scaling), D3D11_INPUT_PER_VERTEX_DATA, 0 },
    { "ROTATION", 0, DXGI_FORMAT_R32_FLOAT, 0, offsetof(d3d11_sprite_t, rotation), D3D11_INPUT_PER_VERTEX_DATA, 0 },
      };
...
#include "d3d_shaders/sprite_sm4.hlsl.h"
         ;
      static const char shader2[] =
#include "d3d_shaders/sprite_sm3.hlsl.h"
         ;
#if IS_LEVEL_93
      if (!d3d11_init_shader(
         d3d11->device, shader2,
         sizeof(shader2), NULL, "VSMain", "PSMain", NULL, desc,
      ...
      if (!d3d11_init_shader(
         d3d11->device, shader2,
         sizeof(shader2), NULL, "VSMain", "PSMainA8", NULL, desc,
...
#else
      if (!d3d11_init_shader(
         d3d11->device, shader1,
         sizeof(shader1), NULL, "VSMain", "PSMain", "GSMain", desc,
         countof(desc), &d3d11->sprites.shader,
         D3D11_FEATURE_LEVEL_HINT_DONTCARE))
      ...
      if (!d3d11_init_shader(
         d3d11->device, shader1,
         sizeof(shader1), NULL, "VSMain", "PSMainA8", "GSMain", desc,
         countof(desc), &d3d11->sprites.shader_font,
         D3D11_FEATURE_LEVEL_HINT_DONTCARE))
      {
         goto error;
...
#include "d3d_shaders/snowflake_sm4.hlsl.h"
...

   {
      ...
      HRESULT hr = d3d11->device->lpVtbl->CreateBlendState(d3d11->device, &blend_desc, &d3d11->blend_enable);
      if (FAILED(hr)) {
         RARCH_ERR("CreateBlendState failed: 0x%08lx", hr);
      }

      blend_desc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
      blend_desc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;

      hr = d3d11->device->lpVtbl->CreateBlendState(d3d11->device, &blend_desc, &d3d11->blend_pipeline);
      if (FAILED(hr)) {
         RARCH_ERR("CreateBlendState failed: 0x%08lx", hr);
      }
      ...
      hr = d3d11->device->lpVtbl->CreateBlendState(d3d11->device, &blend_desc, &d3d11->blend_disable);
      if (FAILED(hr)) {
         RARCH_ERR("CreateBlendState failed: 0x%08lx", hr);
      }
   }

   {
      ...
      HRESULT hr = d3d11->device->lpVtbl->CreateRasterizerState(d3d11->device, &desc, &d3d11->scissor_enabled);
      if (FAILED(hr)) {
         RARCH_ERR("CreateRasterizerState failed: 0x%08lx", hr);
      }

      desc.ScissorEnable = FALSE;

      hr = d3d11->device->lpVtbl->CreateRasterizerState(d3d11->device, &desc, &d3d11->scissor_disabled);
      if (FAILED(hr)) {
         RARCH_ERR("CreateRasterizerState failed: 0x%08lx", hr);
      }
   }
   ...
   video_frame_info_t* video_info)
{
...
#else
   if (d3d11->resize_chain || d3d11->flags & D3D11_ST_FLAG_RESIZE_CHAIN)
#endif
   {
   ...
   d3d11->context->lpVtbl->OMSetBlendState(
      ...
      NULL, D3D11_DEFAULT_SAMPLE_MASK);
   set_primitive_topology(d3d11, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
   {
   ...
   if (d3d11->shader_preset)
   {
      ...
      for (i = 0; i < d3d11->shader_preset->passes; i++)
      {
...

         {
            ...
            // Removed geometry shader for DirectX 9.3 compatibility
#ifndef IS_LEVEL_93
...
#if IS_LEVEL_93
   // From here everything below is TRIANGLELIST on 9.3
   set_primitive_topology(d3d11, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
#endif
