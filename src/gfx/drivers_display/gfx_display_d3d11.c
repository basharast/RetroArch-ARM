   ...
   void* data, unsigned video_width, unsigned video_height)
{
   ...
   int vertex_count = 6; // Two triangles per quad
...

   {
      ...
      HRESULT hr = d3d11->context->lpVtbl->Map(
         d3d11->context, (ID3D11Resource*)d3d11->sprites.vbo, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mapped_vbo);
      ...
      float x = draw->x / (float)d3d11->viewport.Width;
      float y = (d3d11->viewport.Height - draw->y - draw->height) / (float)d3d11->viewport.Height;
      float w = draw->width / (float)d3d11->viewport.Width;
      float h = draw->height / (float)d3d11->viewport.Height;

      // Set up two triangles for the quad directly into sprite
      sprite[0].x = x;          sprite[0].y = y;          sprite[0].u = 0.0f; sprite[0].v = 0.0f;
      sprite[1].x = x + w;      sprite[1].y = y;          sprite[1].u = 1.0f; sprite[1].v = 0.0f;
      sprite[2].x = x;          sprite[2].y = y + h;      sprite[2].u = 0.0f; sprite[2].v = 1.0f;
      sprite[3].x = x;          sprite[3].y = y + h;      sprite[3].u = 0.0f; sprite[3].v = 1.0f;
      sprite[4].x = x + w;      sprite[4].y = y;          sprite[4].u = 1.0f; sprite[4].v = 0.0f;
      sprite[5].x = x + w;      sprite[5].y = y + h;      sprite[5].u = 1.0f; sprite[5].v = 1.0f;

      for (int i = 0; i < 6; ++i)
      {
         sprite[i].w = w;
         sprite[i].h = h;
         sprite[i].s = draw->coords->tex_coord ? draw->coords->tex_coord[2] : 1.0f;
         sprite[i].t = draw->coords->tex_coord ? draw->coords->tex_coord[3] : 1.0f;
         sprite[i].scaling = draw->scale_factor ? draw->scale_factor : 1.0f;
         sprite[i].rotation = draw->rotation;
         for (int k = 0; k < 4; ++k)
         {
            sprite[i].colors[k][0] = draw->coords->color[k * 4 + 0];
            sprite[i].colors[k][1] = draw->coords->color[k * 4 + 1];
            sprite[i].colors[k][2] = draw->coords->color[k * 4 + 2];
            sprite[i].colors[k][3] = draw->coords->color[k * 4 + 3];
         }
      }
      ...
      d3d11->context->lpVtbl->Unmap(d3d11->context, (ID3D11Resource*)d3d11->sprites.vbo, 0);
   }

   {
      ...
      d3d11->context->lpVtbl->PSSetSamplers(
         d3d11->context, 0, 1, &texture->sampler);
   }

   d3d11->context->lpVtbl->Draw(d3d11->context, vertex_count, d3d11->sprites.offset);
