...
#if IS_LEVEL_93
extern char localFolder[1024];
extern void removeSubstring(char* str, const char* sub);
void ConvertColor(UINT32 color, float outColor[4])
{
   outColor[0] = ((color >> 16) & 0xFF) / 255.0f; // Red
   outColor[1] = ((color >> 8) & 0xFF) / 255.0f;  // Green
   outColor[2] = (color & 0xFF) / 255.0f;         // Blue
   outColor[3] = ((color >> 24) & 0xFF) / 255.0f; // Alpha
}
   ...
   unsigned            text_align)
{
   ...
   if (!d3d11 || !font || !msg || !msg_len)
      return;

   // Calculate the required vertices count (6 vertices per glyph)
   unsigned required_vertices = msg_len * 6;

   if (d3d11->sprites.offset + required_vertices > (unsigned)d3d11->sprites.capacity)
      d3d11->sprites.offset = 0;

   // Ensure we have enough space in the buffer
   if (required_vertices > (unsigned)d3d11->sprites.capacity)
      return;
   ...
   for (i = 0; i < msg_len; i++)
   {
      ...
      float px = (x + (glyph->draw_offset_x * scale)) / (float)d3d11->viewport.Width;
      float py = (y + (glyph->draw_offset_y * scale)) / (float)d3d11->viewport.Height;
      float pw = glyph->width * scale / (float)d3d11->viewport.Width;
      float ph = glyph->height * scale / (float)d3d11->viewport.Height;

      float tu = glyph->atlas_offset_x / (float)font->texture.desc.Width;
      float tv = glyph->atlas_offset_y / (float)font->texture.desc.Height;
      float tw = glyph->width / (float)font->texture.desc.Width;
      float th = glyph->height / (float)font->texture.desc.Height;

      float convertedColor[4];
      ConvertColor(color, convertedColor);

      if ((v - (d3d11_sprite_t*)mapped_vbo.pData + 6) > d3d11->sprites.capacity) {
         // Prevent buffer overflow
         break;
      }

      d3d11_sprite_t vertices[6] = {
         { px,     py,     0.0f, 1.0f, tu,       tv,       tw, th, {{convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}}, 1.0f, 0.0f },
         { px + pw, py,     0.0f, 1.0f, tu + tw,  tv,       tw, th, {{convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}}, 1.0f, 0.0f },
         { px,     py + ph, 0.0f, 1.0f, tu,       tv + th,  tw, th, {{convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}}, 1.0f, 0.0f },

         { px,     py + ph, 0.0f, 1.0f, tu,       tv + th,  tw, th, {{convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}}, 1.0f, 0.0f },
         { px + pw, py,     0.0f, 1.0f, tu + tw,  tv,       tw, th, {{convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}}, 1.0f, 0.0f },
         { px + pw, py + ph, 0.0f, 1.0f, tu + tw, tv + th,  tw, th, {{convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}, {convertedColor[0], convertedColor[1], convertedColor[2], convertedColor[3]}}, 1.0f, 0.0f }
      };
   ...
   unsigned            text_align)
{
   ...
   {
      removeSubstring(msg, localFolder);
   }

   if (!font->font_driver->get_line_metrics || !font->font_driver->get_line_metrics(font->font_data, &line_metrics))
   {
      size_t msg_len = strlen(msg);
      while (msg_len > 0) {
         size_t line_len = msg_len;

         for (size_t i = 0; i < msg_len; ++i) {
            if (msg[i] == ' ' && (i * scale) / width > 1.0f) {
               line_len = i;
               break;
            }
         }
         if (line_len > (unsigned)d3d11->sprites.capacity)
            line_len = d3d11->sprites.capacity;

         d3d11_font_render_line(d3d11, font, msg, line_len, scale, color, pos_x, pos_y - lines * line_height, width, height, text_align);
         msg += line_len;
         msg_len -= line_len;
         lines++;
      }
   ...
   for (;;)
   {
      ...
      if (msg_len <= (unsigned)d3d11->sprites.capacity)
         d3d11_font_render_line(d3d11, font, msg, msg_len, scale, color, pos_x, pos_y - (float)lines * line_height, width, height, text_align);
   ...
   const struct font_params* params)
{
   ...
   if (drop_x || drop_y)
   {
      ...
      d3d11_font_render_message(d3d11, font, msg, scale, color_dark,
         x + scale * drop_x / width, y + scale * drop_y / height,
         width, height, text_align);
   }

   d3d11_font_render_message(d3d11, font, msg, scale, color, x, y, width, height, text_align);
}
#else
static void d3d11_font_render_message(
      d3d11_video_t *d3d11,
